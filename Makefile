...
help:
	...
	@echo "build\n\tbuild terrasec binary"
	@echo "cicd\n\tsimulate CI/CD pipeline locally"
	@echo "clean\n\tclean up build"
	@echo "docker-build\n\tbuild terrasec docker image"
	@echo "docker-push\n\tpush terrasec docker image"
	@echo "docker-push-latest\n\tpush terrasec docker image with latest tag"
	@echo "docker-push-latest-tag\n\tpush terrasec docker image with latest release tag"
	@echo "docker-atlantis-build\n\tbuild terrasec_atlantis docker image"
	@echo "docker-atlantis-push\n\tpush terrasec_atlantis docker image"
	@echo "docker-atlantis-push-latest\n\tpush terrasec_atlantis docker image with latest tag"
	@echo "docker-atlantis-push-latest-tag\n\tpush terrasec_atlantis docker image with latest release tag"
	@echo "gofmt\n\tvalidate gofmt"
	@echo "golint\n\tvalidate golint"
	@echo "gomodverify\n\tverify go modules"
	@echo "govet\n\tvalidate govet"
...
build: clean
	...
	go build ${BUILD_FLAGS} -o ${BUILD_DIR}/${BINARY_NAME} cmd/terrasec/main.go
...
# build and push latest terrasec docker image
docker-build-push-latest:
	./scripts/docker-build.sh latest

# build and push release tag terrasec docker image
...
# push latest terrasec docker image
docker-push-latest:
	./scripts/docker-push-latest.sh

# push release tag terrasec docker image
...
# push latest terrasec_atlantis docker image
atlantis-docker-push-latest:
	./scripts/atlantis/docker-push-latest.sh

# push release tag terrasec_atlantis docker image
