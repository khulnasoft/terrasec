...
{{- $ca := genCA ( printf "%s-server-ca" "terrasec" ) 365 }}
{{- $certterrasec := genSignedCert ( printf "%s-server" "terrasec" ) nil $altNames 365 $ca }}
{{- $_ := set . "cert" $certterrasec.Cert -}}
{{- $_ := set . "key" $certterrasec.Key -}}
{{- else }}
  ...
  - name: {{ .Values.webhook.name }}
    ...
    clientConfig:
      service:
        name: {{ .Values.name }}
        namespace: {{ .Release.Namespace }}
        path: {{ .Values.terrasec_webhook_key | printf "/v1/k8s/webhooks/%s/scan/validate" }}
...
# It turns out, webhook doesn't allow the terrasec server pod to come up in case failurePolicy is Fail.
