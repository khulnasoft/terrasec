...
import (
	...
	"github.com/khulnasoft/terrasec/test/e2e/server"
	"github.com/khulnasoft/terrasec/test/e2e/validatingwebhook"
...
var _ = Describe("ValidatingWebhook", func() {
	...
	Describe("terrasec server as validating webhook with various available config options", func() {
		...
		When("validating webhook with default 'k8s-admission-control' config", func() {
			...
			Context("by default validating webhook runs in blind mode", func() {
				...
				It("server should start running on port 9010", func() {
					...
					err := validatingwebhook.CreateTerrasecConfigFile(configFileName, policyRootRelPath, nil)
		...
		When("validating webhook config has 'dashboard' and 'save-requests' is enabled", func() {
			...
			It("server should start running on port 9011", func() {
				...
				err := validatingwebhook.CreateTerrasecConfigFile(configFileName, policyRootRelPath, &terrasecConfig)
				Expect(err).NotTo(HaveOccurred())
