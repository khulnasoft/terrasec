...
const (
	backwardsCompatibilityWarningMessage = "There may be a few breaking changes while working with terraform v0.12 files. For further information, refer to https://github.com/khulnasoft/terrasec/releases/v1.3.0"
)
	...
	Context("terraform is the default iac type", func() {
		...
		Context("default iac version for terraform is v14", func() {
			When("iac version is v12", func() {
				It("terrasec should display the warning message related to version", func() {
					scanArgs := []string{scanUtils.ScanCommand, "-d", iacDir, "-i", "terraform", "--iac-version", "v12"}
					scanUtils.RunScanAndAssertErrorMessage(terrasecBinaryPath, helper.ExitCodeThree, scanUtils.ScanTimeout, backwardsCompatibilityWarningMessage, outWriter, errWriter, scanArgs...)
				})
			})

			When("iac version is v13", func() {
				It("terrasec should not display the warning message related to version", func() {
	...
	Describe("scan iac files violating aws policies against test policy set", func() {
		...
		Context("iac file violates aws_ami policy", func() {
			It("should scan and display violations in human output format", func() {
				...
				scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_human.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
			...
			When("-v flag is used for verbose output", func() {
				It("should display verbose output for human output format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_human_verbose.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is json", func() {
				It("should display violations in json format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_json.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is sarif", func() {
				It("should display violations in sarif format", func() {
					...
					scanUtils.RunScanAndAssertGoldenSarifOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_sarif.txt"), helper.ExitCodeThree, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is json and no iac type is specified", func() {
				Context("when iac type is not specified and a directory is specified, it will be scanned will all iac providers", func() {
					It("should display violations in json format, and should have iac type as 'all'", func() {
						...
						scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_json_all.txt"), helper.ExitCodeFive, false, true, outWriter, errWriter, scanArgs...)
					})
				})
			})
			...
			When("output type is yaml", func() {
				It("should display violations in yaml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_yaml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is xml", func() {
				It("should display violations in xml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_xml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is junit-xml", func() {
				It("should display violations in junit-xml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_junit_xml.txt"), helper.ExitCodeThree, true, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			Context("policy path with only aws policies is supplied", func() {
				...
				It("should display violations", func() {
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfGoldenRelPath, "scanned_with_only_aws_policies.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			Context("policy path with no aws policies is supplied", func() {
				...
				It("should not display any violations and exit with status code 0", func() {
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir}
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfGoldenRelPath, "scanned_with_no_aws_policies.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
		...
		Context("iac file violates aws_db_instance policy", func() {
			...
			When("output type is json", func() {
				It("should display violations in json format", func() {
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
					scanUtils.RunScanAndAssertJSONOutput(terrasecBinaryPath, filepath.Join(tfAwsDBInstanceGoldenRelPath, "aws_db_instance_json.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is yaml", func() {
				It("should display violations in yaml format", func() {
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "yaml"}
					scanUtils.RunScanAndAssertYAMLOutput(terrasecBinaryPath, filepath.Join(tfAwsDBInstanceGoldenRelPath, "aws_db_instance_yaml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("output type is xml", func() {
				It("should display violations in xml format", func() {
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "xml"}
					scanUtils.RunScanAndAssertXMLOutput(terrasecBinaryPath, filepath.Join(tfAwsDBInstanceGoldenRelPath, "aws_db_instance_xml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("--show-passed option is used", func() {
				It("should display passed rules in the output", func() {
					...
					scanUtils.RunScanAndAssertJSONOutput(terrasecBinaryPath, filepath.Join(tfAwsDBInstanceGoldenRelPath, "aws_db_instance_json_show_passed.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
		...
		Context("when --non-recursive flag is not used, all sub folders will be scanned in the specified directory", func() {
			When("output type is json", func() {
				It("should display violations in json format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfAwsAmiGoldenRelPath, "aws_ami_violation_json_recursive.txt"), helper.ExitCodeFive, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
		Context("when --use-terraform-cache flag is used, all remote modules are refered from terraform cache", func() {
			When("when --use-terraform-cache is set with output format json", func() {
				It("should not display any violations", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(terrasecBinaryPath, filepath.Join(tfGoldenRelPath, "terraform_cache_use_in_scan", "terraform_cache_use_in_scan_result.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
	})
