...
var (
	...
	// versionValuePattern is regex for 'value' attribute in junit-xml output (which is terrasec version)
...
// GetTerrasecBinaryPath returns the terrasec binary path
func GetTerrasecBinaryPath() string {
	terrasecBinaryPath := os.Getenv("TERRASEC_BIN_PATH")
	ginkgo.Describe("terrasec binary path should be set for executing tests", func() {
		if terrasecBinaryPath == "" {
			ginkgo.Fail("ensure that TERRASEC_BIN_PATH is set")
		}
	})
	return terrasecBinaryPath
}
