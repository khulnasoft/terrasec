...
const terrasecReadmeURL string = "https://raw.githubusercontent.com/khulnasoft/terrasec/master/README.md"
const filePermissionBits fs.FileMode = 0755

// Run initializes terrasec if not done already
func Run(isNonInitCmd bool) error {
	...
	zap.S().Debug("initializing terrasec")
...
func downloadDefaultPolicies(policyBasePath string) error {
	if !connected(terrasecReadmeURL) {
	...
	zap.S().Debugf("cloning terrasec repo at %s", policyBasePath)
