...
func GetLogger(logLevel, encoding, logDir string, encodingLevel func(zapcore.Level, zapcore.PrimitiveArrayEncoder)) *zap.Logger {
	...
	if logDir != "" {
		...
		if utils.IsWindowsPlatform() {
			zap.RegisterSink(terrasecZapSink, newTerrasecWinFileSink)
			logDirPath = terrasecZapSinkWithSeparator + logDirPath
		}
...
// newTerrasecWinFileSink custom sink to open file on windows
// https://github.com/uber-go/zap/issues/621#issue-350197709 - referred from this issue comment
func newTerrasecWinFileSink(u *url.URL) (zap.Sink, error) {
