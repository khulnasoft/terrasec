...
const (
	defaultPolicyRepoURL     = "https://github.com/khulnasoft/terrasec.git"
	defaultPolicyBranch      = "master"
	defaultPolicyEnvironment = "https://cloud.khulnasoft.com"
)

// ConfigEnvvarName env variable
const ConfigEnvvarName = "TERRASEC_CONFIG"

var (
	defaultPolicyRepoPath = filepath.Join("pkg", "policies", "opa", "rego")
	defaultBasePolicyPath = filepath.Join(utils.GetHomeDir(), ".terrasec")
)
...
func LoadGlobalConfig(configFile string) error {
	...
	var configReader *TerrasecConfigReader
	var err error

	if configReader, err = NewTerrasecConfigReader(configFile); err != nil {
		return err
	}
