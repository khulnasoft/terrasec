...
import (
	...
	"github.com/khulnasoft/terrasec/pkg/k8s/dblogs"
...
var (
	...
	// ErrAPIKeyEnvNotSet indicates K8S_WEBHOOK_API_KEY is not set in terrasec server env
	ErrAPIKeyEnvNotSet = fmt.Errorf("variable K8S_WEBHOOK_API_KEY not set in terrasec server environment")
...
func (w ValidatingWebhook) Authorize(apiKey string) error {
	...
	// API key not set in terrasec env
...
func (w ValidatingWebhook) ProcessWebhook(review admissionv1.AdmissionReview, serverURL string) (*admissionv1.AdmissionReview, error) {
	...
	if err != nil {
		msg := "failed to evaluate terrasec policies"
