...
func scan(cmd *cobra.Command, args []string) error {
	zap.S().Debug("running terrasec in cli mode")
...
func init() {
	...
	scanCmd.Flags().StringVar(&scanOptions.severity, "severity", "", "minimum severity level of the policy violations to be reported by terrasec")
	scanCmd.Flags().StringSliceVarP(&scanOptions.categories, "categories", "", []string{}, "list of categories of violations to be reported by terrasec (example: --categories=\"category1,category2\")")
	...
	scanCmd.Flags().StringVarP(&scanOptions.notificationWebhookURL, "webhook-url", "", "", "webhook URL where Terrasec will send JSON scan report and normalized IaC JSON")
